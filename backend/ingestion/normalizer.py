...
DEPT_PATTERNS = {
    DepartmentEnum.ENGINEERING: [
        r"engineering", r"civil", r"track", r"p-?way", r"permanent\s*way",
        r"bridge", r"works", r"den", r"aen", r"sse/pway"
    ],
    DepartmentEnum.ST: [
        r"s\s*&\s*t", r"signal", r"telecom", r"interlocking",
        r"point\s*machine", r"axle\s*counter", r"track\s*circuit",
        r"sse/sig", r"sse/tele"
    ],
    DepartmentEnum.ELECTRICAL: [
        r"electrical", r"traction", r"ohe", r"trd", r"power",
        r"catenary", r"pantograph", r"substation", r"tss", r"sse/trd"
    ],
...
def normalize_priority(text: str) -> Tuple[int, Optional[str], bool]:
    ...
    if m_p:
        ...
        return (
            3,
            f"Priority {p} specified exceeds Phase 2 range (1-3); normalized to Normal (P3) and flagged for review",
            True,
        )
...
def normalize_table_data(
    ...
    for idx, col in enumerate(header_row):
        ...
        elif c_clean in ["id", "job id", "job_id", "request id", "request_id",
                         "req id", "req_id", "request no", "sl no", "sl_no", "item"]:
            col_map["id"] = idx
        ...
        elif any(k in c_clean for k in ["block_type", "block type", "category"]):
    ...
    results: List[MaintenanceRequest] = []
    ...
    for row_idx, row in enumerate(table[1:]):
        ...
        dept = (
            detect_department(dept_str)
            or detect_department(work_str)
            or detect_department(asset_str)
            or DepartmentEnum.ENGINEERING
        )
...
def normalize_prose_text(
    ...
    train_pattern = (
        r"(?:train|express|freight|mail)\s*(?:no\.?|#)?\s*([A-Za-z0-9\s\-]+?)\s*"
        r"on\s*(?:corridor\s*)?([A-Za-z0-9\-]+)\s*(?:from|dep|departing)?\s*"
        r"(\d{1,2}:\d{2})\s*(?:to|arr|arriving)?\s*(\d{1,2}:\d{2})\s*"
    ...
    for chunk in chunks:
        ...
        corridor = None
        corr_m = re.search(
            r"(?:corridor|section|route|line)\s*[:\s]\s*([A-Za-z0-9\-/\s]+?)(?:\n|,|\.|\s*between|\s*from)",
            chunk_clean,
            re.IGNORECASE
        )
        if corr_m:
        ...
        isolation = "Power Block (OHE)" if (
            "power block" in chunk_clean.lower() or dept == DepartmentEnum.ELECTRICAL
        ) else "None"

        work_type = "Track & Asset Maintenance"
        for candidate in [
            "Rail Grinding", "Track Tamping", "Ballast Cleaning",
            "OHE Catenary Inspection", "Point Machine Replacement",
            "Track Circuit Testing", "Bridge Girder Inspection",
            "Turnout Overhaul", "Insulator Replacement"
        ]:
...
def normalize_train_table(
    ...
    for row in table[1:]:
        ...
        tid = get_col("train_id") or "UNKNOWN_TRAIN"
        t_num = get_col("train_number") or None
        t_name = get_col("train_name") or None
        corridor = get_col("corridor") or "UNSPECIFIED-CORRIDOR"
        dep_str = get_col("departure_time")
        arr_str = get_col("arrival_time")
        t_dep = parse_datetime_flexible(dep_str, base_date)
        t_arr = parse_datetime_flexible(arr_str, base_date)

        if t_dep is None:
            continue
        if t_arr is None:
            t_arr = t_dep + timedelta(hours=1)
        ...
        train_type = get_col("train_type") or "Express"
        speed_val: Optional[float] = None
        if "speed_kmh" in col_map:
            try:
                speed_val = float(re.sub(r"[^\d.]", "", get_col("speed_kmh")))
            except (ValueError, TypeError):
                pass
        ...
        trains.append(TrainMovement(
            ...
            departure_time=t_dep,
            arrival_time=t_arr,
            km_start=min(km_s, km_e),
            km_end=max(km_s, km_e),
...
def process_document_content(doc: DocumentContent, doc_type: Optional[str] = None) -> IngestResponse:
    ...
    if doc_type == "train_movement":
        for table in doc.tables:
            trains = normalize_train_table(table, doc.filename, application_id)
            all_trains.extend(trains)
        if not all_trains and doc.raw_text.strip():
        ...
        if not all_requests or len(doc.raw_text.strip()) > 50:
            ...
            if not all_requests:
                ...
                for pr in prose_reqs:
                    is_duplicate = any(
                        r.corridor == pr.corridor
                        and abs(r.km_start - pr.km_start) < 0.5
                        and r.department == pr.department
    ...
    if not all_requests and not all_trains:
        warnings.append(f"No records could be extracted from {doc.filename}. Please check file layout.")

    return IngestResponse(
        application_id=application_id,
        filename=doc.filename,
        total_extracted=len(all_requests) + len(all_trains),
