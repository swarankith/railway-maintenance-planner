...
export const EditRequestModal: React.FC<EditRequestModalProps> = ({
  ...
  useEffect(() => {
    if (request) {
      // Format datetimes to input string
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {isNew ? 'Manual entry into empty repository' : 'Complete missing fields to verify candidate record'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-4 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Corridor */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Corridor / Section</label>
              <input
                type="text"
                required
            ...
            {/* KM Start */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">KM Start</label>
              <input
                type="number"
                step="0.1"
                required
            ...
            {/* KM End */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">KM End</label>
              <input
                type="number"
                step="0.1"
                required
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Rail Grinding, OHE Overhaul"
                className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">
                Priority Scale (1 = Highest / Emergency, 5 = Lowest / Routine)
              </label>
              <select
                value={formData.priority || 3}
                onChange={(e) => setFormData({ ...formData, priority: parseInt(e.target.value) })}
                className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500 font-semibold"
              >
                <option value={1}>P1 - Critical Safety / Emergency</option>
                <option value={2}>P2 - High Operational Urgency</option>
                <option value={3}>P3 - Standard Maintenance</option>
                <option value={4}>P4 - Routine Inspection</option>
                <option value={5}>P5 - Deferrable Routine Work</option>
              </select>
            </div>
            ...
            {/* Earliest Start (IST) */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Earliest Permitted Start (IST)</label>
              <input
                type="datetime-local"
                required
            ...
            {/* Latest End (IST) */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Latest Permitted End (IST)</label>
              <input
                type="datetime-local"
                required
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">
              Required Machinery & Specialized Teams (comma-separated)
            </label>
            <input
              type="text"
              value={resourceInput}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Isolation Requirement</label>
            <select
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            />
            <label htmlFor="block_shared_allowed" className="text-xs font-medium text-slate-300">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-semibold rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={saving}
              className="flex items-center gap-2 px-5 py-2 text-xs font-semibold rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg shadow-cyan-600/30 transition disabled:opacity-50"
            >
