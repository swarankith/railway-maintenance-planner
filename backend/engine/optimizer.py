...
def resource_identity_clash(left: MaintenanceRequest, right: MaintenanceRequest) -> bool:
    ...
    Asset matching is forgiving: after normalisation, if one asset string is contained
    within the other (e.g., 'Track Section' and 'Track'), they are treated as the same.
    """
    norm_work_l = _norm(left.work_type)
    norm_work_r = _norm(right.work_type)
    norm_asset_l = _norm(left.asset)
    norm_asset_r = _norm(right.asset)

    # If either asset is empty, we cannot detect a clash (should not happen)
    if not norm_asset_l or not norm_asset_r:
        return False

    work_match = norm_work_l == norm_work_r
    asset_match = norm_asset_l == norm_asset_r or norm_asset_l in norm_asset_r or norm_asset_r in norm_asset_l
...
def solve_maintenance_schedule(requests: List[MaintenanceRequest], train_movements: Optional[List[TrainMovement]] = None, mode: str = "recommended") -> SchedulePlan:
    ...
    for r in emergencies:
        ...
        decisions[r.request_id] = RequestDecision(
            ...
            reason="Competing emergencies share this corridor/window; both are isolated for human review." if competing else "Emergency corridor/window isolated pending urgent human sign-off.",
            train_window_checked=False
        )
        # Create an emergency block so it appears in the Gantt chart
        block = MaintenanceBlock(
            block_id=f"BLK-{uuid.uuid4().hex[:6].upper()}",
            corridor=r.corridor,
            scheduled_start=r.earliest_start,
            scheduled_end=r.latest_end,
            duration_minutes=r.duration_minutes,
            km_start=r.km_start,
            km_end=r.km_end,
            request_ids=[r.request_id],
            departments=[r.department],
            resources_allocated=r.required_resources,
            isolation_applied="Disconnection required" if needs_disconnection(r) else "None",
            utilization_score=100,
            time_saved_minutes=0,
            bundling_explanation="Emergency isolated reservation pending human sign-off.",
            requests=[r]
        )
        blocks.append(block)
        occupied.append((r.corridor, r.earliest_start, r.latest_end, [r]))

    # Step 2: Category split and Category B fast path
    ...
    for r in remaining:
        ...
        if not needs_disconnection(r) and not resource_conflict:
            decisions[r.request_id] = RequestDecision(
                ...
                priority=r.priority,
                retry_count=r.retry_count,
                reason="Category B work has no crew/resource conflict, so it is approved without a train-window check.",
                train_window_checked=False
            )
            # Create a block for the fast‑path Category B job
            block = MaintenanceBlock(
                block_id=f"BLK-{uuid.uuid4().hex[:6].upper()}",
                corridor=r.corridor,
                scheduled_start=r.earliest_start,
                scheduled_end=r.earliest_start + timedelta(minutes=r.duration_minutes),
                duration_minutes=r.duration_minutes,
                km_start=r.km_start,
                km_end=r.km_end,
                request_ids=[r.request_id],
                ...
                utilization_score=100,
                time_saved_minutes=0,
                bundling_explanation="Category B fast path: no resource conflict, train-window check skipped.",
                requests=[r]
            )
            blocks.append(block)
        else:
    ...
    # Steps 3‑7 for remaining requests (Category A and conflicted Category B)
    for group in _groups(processing):
        ...
        for i, unit in enumerate(units):
            priority = min(r.priority for r in unit)
            due = min(r.due_date or r.earliest_start.date() for r in unit)

            # True tie: identical priority and due date, AND actual overlap (corridor/time/KM)
            tied = any(
                i != j and
                min(r.priority for r in other) == priority and
                min(r.due_date or r.earliest_start.date() for r in other) == due and
                any(_overlap(r1, r2) for r1 in unit for r2 in other)
            ...
            if free:
                # Check against occupied slots (including emergencies and previously granted units)
                free = not any(
                    c.strip().upper() == unit[0].corridor.strip().upper() and
                    intervals_overlap(start, end, other_start, other_end) and
                    any(km_ranges_overlap(r.km_start, r.km_end, o.km_start, o.km_end) for r in unit for o in other)
                    for c, other_start, other_end, other in occupied
                )

            bundle_id = f"BND-{uuid.uuid4().hex[:6].upper()}" if len(unit) > 1 else None
            if free:
                resources = list(dict.fromkeys(x for r in unit for x in r.required_resources))
                isolation = "Disconnection required" if any(needs_disconnection(r) for r in unit) else "None"
                saved = sum(r.duration_minutes for r in unit) - duration

                blocks.append(MaintenanceBlock(
                    block_id=f"BLK-{uuid.uuid4().hex[:6].upper()}",
                    ...
                    isolation_applied=isolation,
                    utilization_score=100,
                    time_saved_minutes=saved,
                    bundling_explanation=generate_block_explanation(unit[0].corridor, unit, duration, saved, isolation, resources),
                    requests=list(unit)
                ))
                ...
                for r in unit:
                    decisions[r.request_id] = RequestDecision(
                        ...
                        priority=r.priority,
                        bundle_id=bundle_id,
                        bundle_members=[x.request_id for x in unit if x != r],
                        retry_count=r.retry_count,
                        reason="Approved as part of a strict mutually-compatible bundle capped at three requests." if bundle_id else "Approved by the priority walk as the available highest-priority request.",
                        train_window_checked=needs_disconnection(r)
                    )
            else:
                for r in unit:
                    retry = r.retry_count + 1
                    status = "Manual Review" if tied or r.request_id in clashes or retry >= 3 else "Deferred"
                    if tied:
                        reason = "Manual Review: tied competitors have identical priority and due date and overlap on the same corridor/time/KM."
                    elif r.request_id in clashes:
                        reason = "Manual Review: identical work type on the identical asset is an unavoidable resource-identity clash."
                    elif retry >= 3:
                        reason = "Manual Review: retry cap of three reached."
                    else:
                        reason = "Deferred: no safe corridor/train window remained after higher-priority grants."
                    decisions[r.request_id] = RequestDecision(
                        request_id=r.request_id,
                        final_status=status,
                        disconnection_required=needs_disconnection(r),
    ...
    saved = sum(b.time_saved_minutes for b in blocks)
    downtime = sum(b.duration_minutes for b in blocks)
    unresolved = [d for d in decisions.values() if d.final_status in ("Deferred", "Manual Review")]
    efficiency = round(saved / (downtime + saved) * 100, 1) if downtime + saved else 0.0

    total_jobs_completed = sum(1 for d in decisions.values() if d.final_status in ("Approved", "Isolated-Emergency"))
    ...
    return SchedulePlan(
        ...
        blocks=blocks,
        unassigned_requests=[d.request_id for d in unresolved],
        infeasibility_reasons=[d.reason for d in unresolved],
        total_corridor_downtime_minutes=downtime,
