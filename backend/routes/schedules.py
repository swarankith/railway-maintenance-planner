"""
Schedule Optimization & Approval API Endpoints (Phase 2 Final v7).
...
Fixes (v7):
- get_eligible_requests no longer includes Needs-Review or Optimized statuses.
  Only Confirmed, Deferred, and Manual Review are eligible.
- optimize_schedules no longer runs the engine twice; only Plan A is computed.
  alternative_plan is returned as None until a proper alternative strategy exists.
"""
import json
import uuid
...
def get_eligible_requests(db: Session, request_ids: Optional[List[str]] = None) -> List[DBMaintenanceRequest]:
    ...
    - cycle_id IS NULL (brand new) AND status in (Confirmed, Deferred, Manual Review), OR
    - referenced ProcessingCycle.status IN (Approved, Rejected)
      AND final_status IN (Confirmed, Deferred, Manual Review).
    A request whose cycle is still Active is NOT eligible.
    Needs-Review and Optimized statuses are explicitly excluded.
    """
    ...
    eligible: List[DBMaintenanceRequest] = []

    # Statuses allowed to enter the batch engine
    ELIGIBLE_STATUSES = {
    ...
    for r in all_reqs:
        ...
        if not r.cycle_id:
            # Brand new — only eligible statuses
            if r.status in ELIGIBLE_STATUSES:
                eligible.append(r)
        elif r.cycle_id in completed_cycle_ids:
            # From a closed cycle — only leftover eligible statuses
            if r.status in ELIGIBLE_STATUSES:
                eligible.append(r)
...
def optimize_schedules(
    ...
    """
    Triggers the deterministic batch engine over eligible requests & trains (Part E).
    ...
    # Only Plan A (recommended). No alternative plan in this phase.
    plan_a = solve_maintenance_schedule(
        requests, train_movements, mode="recommended", cycle_id=cycle_id
    )
    ...
    res_dict["alternative_plan"] = None
    return res_dict
...
def approve_schedule(
    ...
    """Part G & I: Human approval with immutability check."""
    db_plan = db.query(DBSchedulePlan).filter(DBSchedulePlan.schedule_id == schedule_id).first()
    if not db_plan:
        raise HTTPException(status_code=404, detail=f"Schedule '{schedule_id}' not found.")
...
def reject_schedule(
    ...
    """Part G & I: Human rejection with immutability check."""
    db_plan = db.query(DBSchedulePlan).filter(DBSchedulePlan.schedule_id == schedule_id).first()
    if not db_plan:
        raise HTTPException(status_code=404, detail=f"Schedule '{schedule_id}' not found.")
...
def download_approval_report_pdf(schedule_id: str, db: Session = Depends(get_db)):
    """Part F: Streams the Approval Report PDF."""
    db_plan = db.query(DBSchedulePlan).filter(DBSchedulePlan.schedule_id == schedule_id).first()
    if not db_plan:
        raise HTTPException(status_code=404, detail=f"Schedule '{schedule_id}' not found.")

    audit = db.query(DBApprovalAudit).filter(
        DBApprovalAudit.schedule_id == schedule_id
    ).order_by(DBApprovalAudit.timestamp.desc()).first()
...
def get_schedule_audit(schedule_id: str, db: Session = Depends(get_db)):
    ...
    audits = db.query(DBApprovalAudit).filter(
        DBApprovalAudit.schedule_id == schedule_id
    ).order_by(DBApprovalAudit.timestamp.desc()).all()
    return [
        {
            "id": a.id,
